...
load_dotenv(dotenv_path='../.env') # Carrega o .env da pasta raiz

# --- Passo 2: Importar Ferramentas e Classes ---
from crewai import Agent, Task, Crew, Process
# --- MUDANÇA 1: Importar o LLM específico do LangChain ---
from langchain_google_genai import ChatGoogleGenerativeAI
# --- Importar a ferramenta de busca DuckDuckGo ---
from crewai_tools import DuckDuckGoSearchRunTool

# --- Passo 3: Definir o Cérebro (LLM) usando LangChain ---
print("--- [MAIN] Configurando o cérebro (LLM) LangChain com Gemini... ---")
try:
    # Instancia o Chat Model do LangChain diretamente
    # Passamos a API key explicitamente, pois ele procura por GOOGLE_API_KEY por padrão
    llm_langchain = ChatGoogleGenerativeAI(
        model="gemini/gemini-2.5-flash", # Modelo robusto para raciocínio e ferramentas
        verbose=True, # Ajuda a depurar chamadas do LLM
        temperature=0.5, # Um pouco de criatividade, mas ainda focado
        google_api_key=os.getenv("GEMINI_API_KEY") # Usando a chave do .env
    )
    print("--- [MAIN] Cérebro (LLM) LangChain configurado. ---")
except Exception as e:
    print(f"Erro ao configurar o LLM do LangChain: {e}")
    print("Verifique sua GEMINI_API_KEY no arquivo .env na raiz do projeto.")
    exit()

# --- Instanciar a Ferramenta de Busca DuckDuckGo NATIVA ---
search_tool = DuckDuckGoSearchRunTool()
print("--- [MAIN] Ferramenta de busca DuckDuckGo configurada. ---")
...
destination_specialist = Agent(
  ...
  # --- MUDANÇA 2: Passar o objeto LLM do LangChain ---
  llm=llm_langchain
)
