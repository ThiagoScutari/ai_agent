...
report_compiler = Agent(
  role='Compilador de Relatório de Viagem',
  goal=(
    'Compilar as informações de pesquisa de destino (da Tarefa 1) e os links '
    'do Google Maps (da Tarefa 2) em um relatório final coeso e bem formatado.'
  ),
  backstory=(
    "Você é um editor assistente mestre em organizar informações. "
    "Seu trabalho é pegar os dados brutos dos pesquisadores e especialistas "
    "em localização e formatá-los em um relatório claro, legível e completo "
...
# Tarefa 3 (Compilação Final)
task_compile_report = Task(
  description=(
    "Compile um relatório de viagem final para o cliente sobre {destino_desejado}.\n"
    "Use o resultado da 'task_research_destination' para a descrição e o "
    "resultado da 'task_find_links' para os links.\n\n"
    "O relatório deve seguir EXATAMENTE este formato:\n\n"
    "## Relatório de Viagem: {destino_desejado}\n\n"
    "### 1. Descrição, Curiosidades e Contexto Cultural\n"
    "[Aqui você deve inserir O CONTEÚDO COMPLETO da Tarefa 1: O parágrafo de resumo, "
    "a lista de 3 praias e a lista de 3 atividades culturais.]\n\n"
    "### 2. Links para o Google Maps\n"
    "[Aqui você deve inserir A LISTA COMPLETA de links da Tarefa 2, "
    "mantendo o formato de bullet points com nome e link.]"
  ),
  expected_output=(
    "Um relatório de viagem completo e formatado em markdown, contendo "
    "a descrição do destino (da Tarefa 1) e a lista de links (da Tarefa 2), "
    "seguindo perfeitamente a estrutura solicitada na descrição."
  ),
  agent=report_compiler,
  # O ponto-chave: esta tarefa recebe o contexto das DUAS tarefas anteriores.
  context=[task_research_destination, task_find_links]
)

def run_crew():
    print("--- [MAIN] Montando o Crew... ---")
    travel_crew = Crew(
      agents=[destination_specialist, location_specialist, report_compiler], # <-- ADICIONE O NOVO AGENTE
      tasks=[task_research_destination, task_find_links, task_compile_report], # <-- ADICIONE A NOVA TAREFA
      process=Process.sequential, # Tarefas rodam em sequência
      verbose=True 
    )
    print("--- [MAIN] Crew montado. ---")

    # ... (o resto do seu código run_crew() permanece igual)
